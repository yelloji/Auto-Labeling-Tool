...
async def generate_transformation_preview(
    ...
    try:
        ...
        try:
            ...
            # Resize for preview
            preview_image = resize_image_for_preview(transformed_image, max_size=400)
...
async def generate_preview_with_image_id(
    ...
    try:
        ...
        if not os.path.isabs(image_file_normalized):
            ...
            # Get project root (where main.py is run from)
            import sys
            project_root = os.getcwd() if 'backend' in os.getcwd() else '/workspace/s01-app-1'
            if 'backend' in project_root:
                project_root = os.path.dirname(project_root)  # Go up one level from backend to project root
            image_file_normalized = os.path.join(project_root, image_file_normalized)
