...
class DatabaseDebugger:
    ...
    def get_releases_table(self):
        """Show all releases in the releases table"""
        cursor = self.conn.cursor()
        ...
        for r in releases:
            print(f"\n📦 Release: {r['name']} (ID: {r['id']})")
            print(f"   📁 Project: {r['project_name']}")
            print(f"   📝 Description: {r['description']}")
            print(f"   💾 Export Format: {r['export_format']}")
            print(f"   🖼️  Original Images: {r['total_original_images']} | Augmented: {r['total_augmented_images']}")
            print(f"   🧮 Final Image Count: {r['final_image_count']}")
            print(f"   📂 Output Path: {r['model_path']}")
            print(f"   📅 Created At: {r['created_at']}")

            # Optional: show parsed datasets/config
            try:
                datasets = json.loads(r['datasets_used']) if r['datasets_used'] else []
                print(f"   📚 Datasets Used: {', '.join(datasets) if datasets else 'None'}")
            except Exception as e:
                print(f"   ⚠️  Failed to parse datasets_used: {e}")
