...
const DataAugmentation = ({ datasetId, onClose }) => {
  ...
  const loadPresets = async () => {
    try {
      const data = await augmentationAPI.getPresets();
  ...
  const loadDefaultConfig = async () => {
    try {
      const data = await augmentationAPI.getDefaultConfig();
  ...
  const loadAugmentationJobs = async () => {
    try {
      const data = await augmentationAPI.getJobs(datasetId);
  ...
  const handlePreview = async () => {
    try {
      ...
      const data = await augmentationAPI.generatePreview(datasetId, values);
      setPreviewData(data);
  ...
  const handleCreateAugmentation = async () => {
    try {
      ...
      const jobData = {
        ...
        config: values,
        images_per_original: values.images_per_original,
        apply_to_split: values.apply_to_split,
        preserve_annotations: values.preserve_annotations
      };
      
      await augmentationAPI.createJob(datasetId, jobData);
      loadAugmentationJobs();
      Modal.success({
  ...
  const handleDeleteJob = async (jobId) => {
    try {
      await augmentationAPI.deleteJob(jobId);
      loadAugmentationJobs();
    } catch (error) {
