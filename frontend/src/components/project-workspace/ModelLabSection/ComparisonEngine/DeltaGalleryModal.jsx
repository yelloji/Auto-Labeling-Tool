...
const DeltaGalleryModal = ({
    ...
    const imageUrl = `/api/v1/experiments/${baselineId}/images/${currentItem.image_name}`;
            ...
            <div className="delta-gallery-body" ref={containerRef}>
                ...
                <div className="delta-image-container">
                    ...
                    {imageDim.width > 0 && (
                        ...
                        >
                            {/* Baseline Box (Human Verification) */}
                            {currentItem.bbox && (
                                <g className="baseline-group">
                                    <rect
                                        x={currentItem.bbox[0]}
                                        y={currentItem.bbox[1]}
                                        width={currentItem.bbox[2] - currentItem.bbox[0]}
                                        height={currentItem.bbox[3] - currentItem.bbox[1]}
                                        fill="none"
                                        stroke={type.includes('resolved') ? '#ff4d4f' : '#1890ff'}
                                        strokeWidth="3"
                                        strokeDasharray="8,4"
                                    />
                                    <text
                                        x={currentItem.bbox[0]}
                                        y={currentItem.bbox[1] - 5}
                                        fill={type.includes('resolved') ? '#ff4d4f' : '#1890ff'}
                                        fontSize="14"
                                        fontWeight="bold"
                            ...
                            {/* Challenger Box (If it exists) */}
                            {currentItem.challenger_match && currentItem.challenger_match.bbox && (
                                <g className="challenger-group">
                                    <rect
                                        x={currentItem.challenger_match.bbox[0]}
                                        y={currentItem.challenger_match.bbox[1]}
                                        width={currentItem.challenger_match.bbox[2] - currentItem.challenger_match.bbox[0]}
                                        height={currentItem.challenger_match.bbox[3] - currentItem.challenger_match.bbox[1]}
                                    ...
                                    <text
                                        x={currentItem.challenger_match.bbox[0]}
                                        y={currentItem.challenger_match.bbox[1] - 5}
                                        fill="#52c41a"
                                        fontSize="14"
                                        fontWeight="bold"
                                        style={{ textShadow: '1px 1px 2px black' }}
