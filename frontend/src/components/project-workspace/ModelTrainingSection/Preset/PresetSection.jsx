      ...
      {isDeveloper && (
        <Collapse defaultActiveKey={["opt", "loss", "aug"]}>
            ...
            <Row gutter={12}>
              <Col span={12}>
                ...
                <Form.Item label="Final LR Factor (lrf)" tooltip="Final LR = lr0 × lrf (cosine schedule end)">
                  <InputNumber min={0} max={1} step={0.001} placeholder={0.01} value={lrf} onChange={(v) => onChange({ lrf: v })} />
                </Form.Item>
                <Form.Item label="Momentum" tooltip="Optimizer momentum / Adam beta1">
                  <InputNumber min={0} max={1} step={0.001} placeholder={0.937} value={momentum} onChange={(v) => onChange({ momentum: v })} />
                </Form.Item>
                <Form.Item label="Weight Decay" tooltip="L2 regularization strength">
                  <InputNumber min={0} step={0.0001} placeholder={0.0005} value={weight_decay} onChange={(v) => onChange({ weight_decay: v })} />
                </Form.Item>
              </Col>
              <Col span={12}>
                ...
                <Form.Item label="Save Period (epochs)" tooltip="Checkpoint every N epochs; -1 disables">
                  <InputNumber min={-1} placeholder={-1} value={save_period} onChange={(v) => onChange({ save_period: v })} />
                </Form.Item>
                <Form.Item label="Workers" tooltip="Data loader workers">
                  <InputNumber min={0} placeholder={8} value={workers} onChange={(v) => onChange({ workers: v })} />
                </Form.Item>
              </Col>
            </Row>
            <Row gutter={12}>
          ...
          <Collapse.Panel header="Loss Weights" key="loss">
            <Row gutter={12}>
              <Col span={8}><Form.Item label="Box" tooltip="Weight of bounding-box regression loss"><InputNumber min={0} step={0.1} placeholder={10.0} value={box} onChange={(v) => onChange({ box: v })} /></Form.Item></Col>
              <Col span={8}><Form.Item label="Cls" tooltip="Weight of classification loss"><InputNumber min={0} step={0.1} placeholder={0.3} value={cls} onChange={(v) => onChange({ cls: v })} /></Form.Item></Col>
              <Col span={8}><Form.Item label="DFL" tooltip="Weight of distribution focal loss (box quality)"><InputNumber min={0} step={0.1} placeholder={2.0} value={dfl} onChange={(v) => onChange({ dfl: v })} /></Form.Item></Col>
            </Row>
          </Collapse.Panel>
          ...
          <Collapse.Panel header="Validation" key="val">
            <Row gutter={12}>
              <Col span={12}><Form.Item label="Val IoU" tooltip="IoU threshold used in validation"><InputNumber min={0} max={1} step={0.01} placeholder={0.5} value={val_iou} onChange={(v) => onChange({ val_iou: v })} /></Form.Item></Col>
              <Col span={12}><Form.Item label="Val Plots" tooltip="Save validation plots"><Switch checked={val_plots} onChange={(v) => onChange({ val_plots: v })} /></Form.Item></Col>
            </Row>
          </Collapse.Panel>
        </Collapse>
      )}
