...
const LiveTrainingDashboard = ({ metrics }) => {
    ...
    return (
        <div style={{ padding: 0 }}>
                                ...
                                <Progress
                                    ...
                                    format={() => (
                                        <div style={{ color: '#fff', fontSize: 12 }}>
                                            <div style={{ fontWeight: 'bold', fontSize: 16 }}>{training.epoch}</div>
                                            <div style={{ fontSize: 10 }}>/{training.total_epochs}</div>
                                        </div>
                                    )}
                                />
            ...
            {/* Section 2: Epoch Report (Scorecard) */}
            {validation.box_map50 !== undefined && (
                <Row gutter={[12, 12]} style={{ marginBottom: 12 }}>
                    <Col span={12}>
                        ...
                        >
                            <Statistic
                                value={(validation.box_map50 * 100).toFixed(1)}
                                suffix="%"
                                valueStyle={{ color: '#1890ff', fontSize: 28, fontWeight: 'bold' }}
                            />
                            <div style={{ fontSize: 11, color: '#888', marginTop: 4 }}>
                                P: {(validation.box_p * 100).toFixed(1)}% | R: {(validation.box_r * 100).toFixed(1)}%
                            </div>
                        </Card>
                    </Col>
                    <Col span={12}>
                        ...
                        >
                            <Statistic
                                value={(validation.mask_map50 * 100).toFixed(1)}
                                suffix="%"
                                valueStyle={{ color: '#722ed1', fontSize: 28, fontWeight: 'bold' }}
                            />
                            <div style={{ fontSize: 11, color: '#888', marginTop: 4 }}>
                                P: {(validation.mask_p * 100).toFixed(1)}% | R: {(validation.mask_r * 100).toFixed(1)}%
                            </div>
                        </Card>
                    </Col>
                </Row>
            ...
            {/* Section 3: Class Grid (Hero + Class Cards) */}
            {validation.class === 'all' && (
                <Card
                    size="small"
                ...
                >
                    <Row gutter={[12, 12]}>
                        <Col span={12}>
                            <Progress
                                type="dashboard"
                                percent={Math.round(validation.box_map50 * 100)}
                                strokeColor="#1890ff"
                        ...
                        <Col span={12}>
                            <Progress
                                type="dashboard"
                                percent={Math.round(validation.mask_map50 * 100)}
                                strokeColor="#722ed1"
                                format={(percent) => `${percent}%`}
                            />
                    ...
                    <Row gutter={[8, 8]} style={{ marginTop: 12 }}>
                        <Col span={12}>
                            <Tag icon={<DatabaseOutlined />} color="blue">Images: {validation.images}</Tag>
                        </Col>
                        <Col span={12}>
                            <Tag icon={<ThunderboltOutlined />} color="green">Instances: {validation.instances}</Tag>
                        </Col>
                    </Row>
                </Card>
            )}
            ...
            {/* Empty State */}
            {!training.epoch && !validation.box_map50 && (
                <div style={{ textAlign: 'center', padding: 24, color: '#888' }}>
                    <ThunderboltOutlined style={{ fontSize: 48, color: '#d9d9d9' }} />
                    <div style={{ marginTop: 12 }}>Training metrics will appear here once training starts</div>
                </div>
            )}
        </div>
    );
