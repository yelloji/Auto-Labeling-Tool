...
const UploadSection = ({ projectId }) => {
  ...
  const handleFileSelect = () => {
    // Directly trigger file input regardless of batch name or tags
  ...
  const uploadProps = {
    ...
    customRequest: async ({ file, onSuccess, onError, onProgress }) => {
      try {
        // Create a batch name if not provided
        ...
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          ...
          <input
            ...
            onChange={async (e) => {
              ...
              try {
                // Use the uploadMultipleFiles function for better performance with multiple files
                if (files.length > 1) {
