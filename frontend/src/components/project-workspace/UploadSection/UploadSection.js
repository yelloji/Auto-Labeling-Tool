...
const UploadSection = ({ projectId }) => {
  ...
  const loadAvailableDatasets = async () => {
    try {
      const datasets = await projectsAPI.getDatasets(projectId);
  ...
  const uploadMultipleFiles = async (files, batchNameToUse) => {
    ...
    try {
      const response = await fetch(`/api/projects/${projectId}/upload-batch`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        } else if (result.results && result.results.errors) {
