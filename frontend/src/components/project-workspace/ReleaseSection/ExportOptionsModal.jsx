...
const ExportOptionsModal = ({ visible, onCancel, releaseData, onExport }) => {
  ...
  const taskTypes = {
    classification: {
      ...
      formats: ['csv', 'json', 'tfrecord'],
      icon: '🏷️'
    },
    object_detection: {
      name: 'Object Detection',
      description: 'Bounding boxes around objects',
      formats: ['yolo', 'coco', 'pascal_voc', 'tfrecord'],
      icon: '📦'
    },
    segmentation: {
      name: 'Instance Segmentation',
      description: 'Pixel-level object masks',
      formats: ['coco', 'pascal_voc', 'cityscapes'],
      icon: '🎨'
    }
  };

  const formatDescriptions = {
    yolo: 'YOLOv5/v8 format - txt files with normalized coordinates',
    coco: 'COCO JSON format - industry standard for object detection',
    pascal_voc: 'Pascal VOC XML format - traditional computer vision format',
    tfrecord: 'TensorFlow Record format - optimized for TensorFlow training',
    csv: 'Comma-separated values - simple tabular format',
    json: 'JSON format - structured data format',
    cityscapes: 'Cityscapes format - specialized for segmentation'
  };
  ...
  const handleExport = async () => {
    try {
      ...
      setTimeout(() => {
        ...
        setDownloadUrl(`/api/v1/releases/${releaseData?.id}/download?format=${values.format}`);
