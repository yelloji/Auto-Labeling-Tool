...
function findButtonByText(text, root = document) {
  return Array.from(root.querySelectorAll('button'))
    ...
    .find(btn => isVisibleElement(btn) && btn.textContent?.trim() === text);
}
...
function clickReleaseAction({ text, selector }) {
  ...
  if (selector) {
    for (const root of roots) {
      ...
      if (el && isVisibleElement(el) && !el.closest('.guide-bot-wrapper')) {
        el.click();
        return true;
      }
    }
...
function isPreviewOpen(state) {
  if (typeof state?.previewVisible === 'boolean') return state.previewVisible;
  return isPreviewVisible();
}

function isReleaseImageViewerOpen(state) {
  if (typeof state?.imageViewerVisible === 'boolean') return state.imageViewerVisible;
  return !!findButtonByText('Fit to Screen') && !!findTextNode(/^Previous$/);
}
...
export function checkReleasePageState(lang, refs, setters) {
  ...
  if (isReleaseImageViewerOpen(state)) {
    const text = lang === 'it'
      ? 'Questa e la vista ingrandita di una immagine generata dalla release. Qui puoi controllare split, annotazioni, zoom e passare alle altre immagini finali.'
      : 'This is the enlarged view of one image generated by the release. Here you can review the split, annotations, zoom, and move through the other final images.';
    return makeState('release-image-viewer', text, [lang === 'it' ? 'Cosa vedo qui?' : 'What am I seeing here?', getBackLabel(lang)], 'release-image-viewer');
  }
...
export function handleReleaseAnswer(step, value, lang, refs, setters) {
  ...
  if (step === 'release-ready-no-tools') {
    ...
    if (value === rebalanceHelpLabel) {
      addMessage('bot',
        lang === 'it'
          ? 'Qui esistono due rebalance diversi. Quello vicino a Dataset Statistics ribilancia Train, Validation e Test sull intero pool della release; quello nel dettaglio di un dataset ribilancia solo quel singolo dataset completato.'
          : 'There are two different rebalances here. The one near Dataset Statistics rebalances Train, Validation, and Test across the whole release pool, while the one in a dataset detail view rebalances only that single completed dataset.',
    ...
    if (value === addBasicLabel) {
      requestReopen();
      clickReleaseAction({ text: 'Add Basic Transformation' });
      closeBot();
      return;
    }
  }
  ...
  if (step === 'release-page-help') {
    ...
    if (value === rebalanceHelpLabel) {
      addMessage('bot',
        lang === 'it'
          ? 'Il rebalance principale si trova nel blocco Dataset Statistics e rimescola gli split su tutti i dataset completati idonei alla release. Il rebalance di dataset invece si trova nel dettaglio di un dataset e lavora solo su quel singolo batch completato.'
          : 'The main rebalance is in Dataset Statistics and reshuffles splits across all completed datasets eligible for the release. Dataset rebalance is inside a dataset detail view and works only on that single completed batch.',
        { inputType: 'buttons', options: [backLabel], step: 'release-rebalance-help' }
      );
      return;
    }
  ...
  if (step === 'release-tools-added') {
    ...
    if (value === continueConfigLabel) {
      requestReopen();
      clickReleaseAction({ text: 'Continue to Release Configuration' });
      closeBot();
      return;
    }
  }
  ...
  if (step === 'release-config-open') {
    ...
    if (value === previewActionLabel) {
      clickReleaseAction({ text: 'Preview Output' });
      return;
    }
  }
  ...
  if (step === 'release-preview-open') {
    ...
    if (value === createReleaseLabel) {
      clickReleaseAction({ text: 'Create Release' });
      return;
    }
  }
  ...
  if (step === 'release-image-viewer') {
    const viewerHelpLabel = lang === 'it' ? 'Cosa vedo qui?' : 'What am I seeing here?';
    if (value === viewerHelpLabel) {
      addMessage('bot',
        lang === 'it'
          ? 'Questa finestra mostra una immagine finale generata dalla release in formato grande. A destra puoi controllare file, split, annotazioni e zoom; in alto puoi usare Previous e Next per passare alle altre immagini.'
          : 'This window shows one final release image in a large view. On the right you can review the file, split, annotations, and zoom, while the top bar lets you use Previous and Next to move through the other images.',
        { inputType: 'buttons', options: [backLabel], step: 'release-image-viewer-help' }
      );
      return;
    }
    if (value === backLabel) {
  ...
  if (step === 'release-details-view') {
    ...
    if (value === downloadZipLabel) {
      clickReleaseAction({ text: 'Download ZIP' });
      return;
    }
    if (value === createNewReleaseLabel) {
      requestReopen();
      clickReleaseAction({ text: 'Create New Release' });
      closeBot();
      return;
    }
    if (value === backToHistoryLabel) {
      requestReopen();
      clickReleaseAction({ text: 'Back to Release History' });
      closeBot();
      return;
    }
  }
  ...
  if (step === 'release-details-help') {
    ...
    if (value === imagesHelp) {
      addMessage('bot',
        lang === 'it'
          ? 'Queste sono le immagini finali generate dalla release. Qui puoi controllare anteprima, split assegnato e annotazioni risultanti; cliccando una miniatura apri anche la vista grande per ispezionarla meglio.'
          : 'These are the final images generated for the release. Here you can review the preview, assigned split, and resulting annotations; clicking a thumbnail also opens the large viewer for closer inspection.',
  ...
  if (step === 'release-tool-config') {
    ...
    if (value === applyTransformationLabel) {
      clickReleaseAction({ selector: '.apply-transformation-button', text: 'Apply Transformation' });
      return;
    }
  }
